...
object DlnaDiagnostics {
    ...
    private const val TAG = "WFAS-DLNA"
    private val entries = ArrayDeque<String>()
    private val lock = Any()
    private val stampFormat = java.text.SimpleDateFormat("HH:mm:ss", Locale.ROOT)
    private val fullFormat = java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.ROOT)

    fun record(tag: String, message: String) {
        val stamp = synchronized(stampFormat) { stampFormat.format(java.util.Date()) }
        val line = "[$stamp][$tag] ${message.take(4000)}"
        synchronized(lock) {
            entries.addLast(line)
            while (entries.size > MAX_ENTRIES) entries.removeFirst()
        }
        android.util.Log.d(TAG, "[$tag] ${message.take(400)}")
    }
    ...
    fun report(): String {
        val head = buildString {
            ...
            appendLine("generated: ${synchronized(fullFormat) { fullFormat.format(java.util.Date()) }}")
    ...
    fun get(url: String, timeoutMs: Int = DlnaConst.READ_TIMEOUT_MS): ByteArray? {
        ...
        return try {
            ...
            if (connection.responseCode !in 200..299) {
                ...
                connection.inputStream.use { DlnaStreams.readLimited(it, DlnaConst.MAX_XML_BYTES) }
            }
        } catch (e: Exception) {
        ...
        return try {
            ...
            val responseBytes = runCatching {
                (if (status in 200..299) connection.inputStream else connection.errorStream)
                    ?.use { DlnaStreams.readLimited(it, DlnaConst.MAX_XML_BYTES) }
    ...
    fun install(context: android.content.Context) {
        synchronized(guard) {
            ...
            lock = runCatching {
                ...
                wifi.createMulticastLock("wfas_dlna_ssdp").apply { setReferenceCounted(true) }
            }.getOrNull()
        }
    }

    fun <T> withLock(block: () -> T): T {
        val held = synchronized(guard) {
            val current = lock
            if (current != null) {
                runCatching { current.acquire() }.isSuccess.also { if (it) holders++ }
            } else false
        }
        try {
            return block()
...
object DlnaSsdp {
    ...
    ): List<String> = DlnaMulticastLock.withLock { discoverInternal(preferred, perAddressTimeoutMs) }

    private fun discoverInternal(
        preferred: NetworkInterface?,
        perAddressTimeoutMs: Int
    ): List<String> {
